	...
	if (Token::check(Input::get('token'))) {
		...
		if ($validation->passed()) {
			...
			try {
				...
				Session::flash('tickets', $e);
				Redirect::to();
			}
			Session::flash('tickets', 'Transactie toegevoegd');
			Redirect::to('tickets.php#form');
...
<div id="content">
	...
	<?php
	if (Session::exists('tickets')) {
		echo "
			<div>
				<h3><center>Opmerkingen</center></h3>
				<p>" . Session::flash('tickets') . "</p>
			</div>
			<div class='clear'></div>
		";
	}
				...
				?>
						<form action="" method="post" id="form" autocomplete="off">
							<tr>
								<td>
									<div>
										<input type="hidden" name="transactie" value="<?php echo count($data); ?>">T<?php echo count($data); ?>
									</div>
								</td>
								<td>
