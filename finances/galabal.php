	...
	if (Token::check(Input::get('token'))) {
		...
		if ($validation->passed()) {
			...
			try {
				...
				Session::flash('galabal', $e);
				Redirect::to();
			}
			Session::flash('galabal', 'Transactie toegevoegd');
			Redirect::to('galabal.php#form');
...
<div id="content">
	...
	<?php
	if (Session::exists('galabal')) {
		echo "
			<div>
				<h3><center>Opmerkingen</center></h3>
				<p>" . Session::flash('galabal') . "</p>
			</div>
			<div class='clear'></div>
		";
	}
				...
				?>
						<form action="" method="post" autocomplete="off" id="form">
							<tr>
								<td>
									<div>
										<input type="hidden" name="transactie" value="<?php echo count($data); ?>">G<?php echo count($data); ?>
									</div>
								</td>
								<td>
